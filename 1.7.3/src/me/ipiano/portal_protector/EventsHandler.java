...
public class EventsHandler implements Listener{
    ...
    public void portalTeleport(PlayerPortalEvent event){
            ...
            /*if(plugin.getProperties().getBoolean(anyBlockPath)){
                Location[] nearby = getNearbyBlocks(event.getTo(), 4);
                for (int i=0; i<nearby.length; i++){
                    if(inRangeOfPortal(nearby[i], 1,0,0) || inRangeOfPortal(nearby[i], 0,0,1)){
                        if(nearby[i].getBlock().)
                    }
                }
            }*/
    ...
    public Location[] getNearbyBlocks(Location location, int radius){
    ...
    public void notifyPlayer(Player player, String message){
        message = ("[" + ChatColor.DARK_PURPLE + "PortalProtector" + ChatColor.WHITE + "] - " + message);
        try{
